    ...
    void testShouldAcceptMcpServerConfigurationOnSessionCreate() throws Exception {
        ...
        try (CopilotClient client = ctx.createClient()) {
            CopilotSession session = client.createSession(
                    new SessionConfig().setOnPermissionRequest(PermissionHandler.APPROVE_ALL).setMcpServers(mcpServers))
                    .get();
    ...
    void testShouldAcceptMcpServerConfigurationOnSessionResume() throws Exception {
        ...
        try (CopilotClient client = ctx.createClient()) {
            ...
            CopilotSession session2 = client.resumeSession(sessionId, new ResumeSessionConfig()
                    .setOnPermissionRequest(PermissionHandler.APPROVE_ALL).setMcpServers(mcpServers)).get();
        ...
        try (CopilotClient client = ctx.createClient()) {
            CopilotSession session = client
                    .createSession(new SessionConfig().setOnPermissionRequest(PermissionHandler.APPROVE_ALL)
                            .setMcpServers(mcpServers).setCustomAgents(customAgents))
                    .get();
